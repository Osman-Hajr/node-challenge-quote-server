...
app.get("/", function(request, response) {
  response.send("Osman's Quote Server!  Ask me for /quotes/random, /quotes, /quotes/search, /search/filtered/quotes or /search/filtered/author");
});

//START OF YOUR CODE...
app.get("/quotes", function(request, response) {
  const allQuotes = quotes.map(
    q => `{"quote": "${q.quote}", "author": "${q.author}"}`
  );
